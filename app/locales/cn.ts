...
const cn = {
  ...
  Settings: {
    ...
    Store: {
    "DefaultTopic": "新的聊天",
    "BotHello": "## 🎉 欢迎来到蓝衫科技Midjourney 😎\n\n### 基础功能：\n\n1. **设置中打开Midjourney图像代理**  \n   通过设置，可以启用或禁用Midjourney图像代理功能。\n\n2. **使用 `/mj` 命令**  \n   在输入框中以 `/mj` 开头输入您的绘画描述，即可进行创建绘画。  \n   例如：`/mj a dog`\n\n### 融合图片功能:\n\n- **文生图**  \n  直接输入图像描述，输出四张根据描述生成的图像。\n\n- **垫图**『需上传1张图像』  \n  输入图像微调描述，输出根据描述调整后的上传图像。\n\n- **混图**『需上传2-5张图像』  \n  根据所有上传图像的内容和风格，输出四张生成的图像。\n\n- **识图**『需上传1张图像』  \n  根据上传图像，输出相应的英文文字描述。\n\n### 参数说明（生成图像后才会出现）:\n\n- **U**  \n  放大四张生成图像中的其中一张，从上到下从左到右，依次排序为1、2、3、4.\n\n- **V**  \n  根据四张生成图像中的其中一张，再生成四张相似的图像，从上到下从左到右，依次排序为1、2、3、4.\n\n- **Z**  \n  将原始图像进行缩放焦距(有1.5倍和2倍的选项)，生成新的四张图像。\n\n- **Vary**  \n  将原始图像进行变化(有强烈变化Strong和微小变化Subtle的选项)，生成新的四张图像。\n\n- **Pan**  \n  将原始图像进行定向扩展（有上下左右的选项），生成新的四张图像。",
    "Error": "出错了，稍后重试吧",
    Prompt: {
